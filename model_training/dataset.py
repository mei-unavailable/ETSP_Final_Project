...
class BrainToTextDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter out days with no trials
        self.trial_indicies = {d: info for d, info in trial_indicies.items() if len(info['trials']) > 0}
        if len(self.trial_indicies) < len(trial_indicies):
            print(f"Warning: {len(trial_indicies) - len(self.trial_indicies)} days were removed from {split} set because they had no trials.")
            
        self.n_days = len(self.trial_indicies.keys())
        
        if self.n_days == 0:
             raise ValueError(f"No valid trials found in the {split} dataset. Please check dataset_dir and session files.")

        self.feature_subset = feature_subset

        # Calculate total number of trials in the dataset
        for d in self.trial_indicies:
            self.n_trials += len(self.trial_indicies[d]['trials'])
