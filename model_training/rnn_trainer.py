...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        if args['mode'] == 'train':
            ...
            # Update checkpoint_dir if it is a subdirectory of output_dir
            if 'checkpoint_dir' in args and args['checkpoint_dir'].startswith(original_output_dir):
                args['checkpoint_dir'] = args['checkpoint_dir'].replace(original_output_dir, args['output_dir'], 1)
        ...
        if model_type == 'conformer':
            conformer_args = dict(
                ...
                n_heads = self.args['model'].get('n_heads', 4),
    ...
    def train(self):
        ...
        for i, batch in enumerate(tqdm(self.train_loader, desc="Training", unit="batch")):
            ...
            if i % self.args['batches_per_val_step'] == 0 or i == ((self.args['num_training_batches'] - 1)):
                ...
                if self.args['log_individual_day_val_PER']:
                    for day in val_metrics['day_PERs'].keys():
                        ...
                        if total_seq_len > 0:
                            day_per = val_metrics['day_PERs'][day]['total_edit_distance'] / total_seq_len
                        else:
                            day_per = 1.0
                        self.logger.info(f"{self.args['dataset']['sessions'][day]} val PER: {day_per:0.4f}")
